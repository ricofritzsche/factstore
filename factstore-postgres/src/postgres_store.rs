...
impl PostgresStore {
        ...
        F: FnOnce(PgPool) -> Fut + Send + 'static,
    {
        ...
        let worker_thread = thread::Builder::new()
            .name(format!("factstore-postgres-{operation}"))
            .spawn(move || {
...
fn run_delivery_thread(
    ...
    let runtime = match Builder::new_current_thread().enable_all().build() {
        ...
        Err(error) => {
            eprintln!(
                "factstore-postgres delivery runtime could not start: {}",
                error
            );
            return;
        }
    ...
    let pool = match runtime.block_on(async {
        ...
        Err(error) => {
            eprintln!(
                "factstore-postgres delivery pool could not connect: {}",
                error
            );
            return;
        }
                        ...
                        DeliveryOutcome::Succeeded {
                            ...
                            if let Some(durable_stream_id) = durable_stream_id {
                                if let Err(error) = runtime.block_on(update_durable_stream_cursor(
                                    ...
                                    eprintln!(
                                        "factstore-postgres durable cursor update failed after delivery for stream {}: {}",
...
fn bootstrap_connection(connection_string: &str) -> Result<(), sqlx::Error> {
    ...
    let bootstrap_thread = thread::Builder::new()
        .name("factstore-postgres-bootstrap".to_owned())
        .spawn(move || {
            let runtime = Builder::new_current_thread()
                .enable_all()
                .build()
