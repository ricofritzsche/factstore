...
fn empty_append_emits_no_delivery() {
    ...
    assert_eq!(error, factstore::EventStoreError::EmptyAppend);
...
fn panicking_handler_does_not_roll_back_append_success() {
    ...
    let _panicking_subscription = store
        .stream_all(Arc::new(|_| -> Result<(), factstore::StreamHandlerError> {
...
fn recording_handle(
    delivery_log: Arc<Mutex<Vec<Vec<factstore::EventRecord>>>>,
) -> factstore::HandleStream {
...
fn assert_no_delivery(delivery_log: &Arc<Mutex<Vec<Vec<factstore::EventRecord>>>>) {
